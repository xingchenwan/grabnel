...
class WeisfeilerLehman(Kernel):
    ...
    def transform_parse(self, X):
        """Extract the feature vector of X in the eval mode (i.e. X is not taken as the training dataset)"""
        ...
        self._inv_labels_transform = dict()
        self._inv_labels_transform[0] = WL_labels_inverse
        ...
        def generate_graphs(WL_labels_inverse, nl):
            ...
            for i in range(1, self._n_iter):
                ...
                for j in range(nx):
                    ...
                    new_graphs.append([Gs_ed[j], new_labels])
                self._inv_labels_transform[i] = WL_labels_inverse
                yield new_graphs

        transformed_graphs = [g for g in generate_graphs(WL_labels_inverse, nl)]
